...
class Controller(tk.Frame):
    def __init__(self, master=None):
        ...
        tk.Frame.__init__(self, master, relief=tk.SUNKEN, bd=2, width=600, height=400)
        self.pack()
        ...
        # buttons
        self.addtodo = tk.Button(self,text="Add To Do", command=self.add_cb)
        self.edittodo = tk.Button(self,text="Edit To Do", command=self.edit_cb)
        self.removetodo = tk.Button(self, text="Remove To Do", command=self.rm_cb)
    ...
    def handler(self):
        if self.todolist.needs_saving() == True:
            if tk.messagebox.askyesno(title='Save Changes', message='Unsaved changes. Save?'):
    ...
    def askopenfile(self):
        ...
        if filename:
            ...
            try:
                ...
                tk.messagebox.showinfo("File Error", "Invalid file format.")
                return
    ...
    def add_cb(self):
        self.todoentry = tk.Toplevel()
        self.todoentry.title("Todo Entry")
        namelabel = tk.Label(self.todoentry, text="Name:")
        # namelabel.pack(side="left")
        namelabel.place(x=20, y=20)
        self.namearea = tk.Text(self.todoentry, height=1, width=15)
        # namearea.pack()
        self.namearea.place(x=70, y=20)

        datelabel = tk.Label(self.todoentry, text="Date:")
        # datelabel.pack(side="left")
        datelabel.place(x=20, y=50)
        self.datearea = tk.Text(self.todoentry, height=1, width=15)
        # datearea.pack()
        self.datearea.place(x=70, y=50)

        cancelbutton = tk.Button(self.todoentry, text="Cancel",  command=self.cancel)
        # cancelbutton.pack()
        cancelbutton.place(x=30, y=80)

        okbutton = tk.Button(self.todoentry, text="OK",  command=self.ok)
        # okbutton.pack()
    ...
    def ok(self):
        name = self.namearea.get("1.0", tk.END)
        date = self.datearea.get("1.0", tk.END)
        # print name
        # print date
        try:
    ...
    def edit_cb(self):
        ...
        if len(items) == 0:
            tk.messagebox.showinfo("Selection Error", "No item selected")
        ...
        namelabel.place(x=20, y=20)
        self.namearea = tk.Text(self.todoentry, height=1, width=15)
        self.namearea.insert(tk.INSERT, todoitem.get_name())
        # namearea.pack()
        self.namearea.place(x=70, y=20)
        ...
        datelabel.place(x=20, y=50)
        self.datearea = tk.Text(self.todoentry, height=1, width=15)
        self.datearea.insert(tk.INSERT, todoitem.get_date())
        # datearea.pack()
        self.datearea.place(x=70, y=50)
        ...
        okbutton = tk.Button(self.todoentry, text="OK",  command=self.editok)
        # okbutton.pack()
        okbutton.place(x=120, y=80)
